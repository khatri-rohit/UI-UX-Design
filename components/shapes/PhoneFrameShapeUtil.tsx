...
export class PhoneFrameShapeUtil extends ShapeUtil<PhoneFrameShape> {
  ...
  getDefaultProps() {
    return { w: 200, h: 380, screenName: '', platform: 'mobile', content: '', state: 'skeleton', srcdoc: '' }
  }
...
function PhoneFrameShapeComponent({ shape }: { shape: PhoneFrameShape }) {
  ...
  const statusBg = state === 'done' ? '#dcfce7' : state === 'error' ? '#fee2e2' : '#e2e8f0'
  const statusText = state === 'done' ? '#166534' : state === 'error' ? '#991b1b' : '#334155'
...
function SkeletonScreen() {
  return (
    <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 10, background: '#fff' }}>
...
function StreamingScreen() {
  return (
    <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 7, background: '#fff' }}>
      <div
        style={{
          width: 48,
          height: 5,
      ...
      />
      <div style={{ fontSize: 10, color: '#64748b', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }}>
        Receiving code...
      </div>
    </div>
  )
